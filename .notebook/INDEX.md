...
| [web-bruno-receivables-workbench-flow.md](web-bruno-receivables-workbench-flow.md) | psychology, web-bruno, financial, receivables, flow | Bruno receivables mix due session rows with protocol-sale rows, but batch reminder/mark-paid actions only operate on the overdue appointment contract from `/api/psychology/sessions` |
| [web-bruno-protocol-payment-ledger-flow.md](web-bruno-protocol-payment-ledger-flow.md) | psychology, web-bruno, api, protocols, financial, payments, ledger, flow | Protocol sale finance now derives from child payment rows, with separate create/edit/add-payment API commands and Bruno-specific payment dialogs |
...
| [web-bruno-appointments-filter-normalization-gotcha.md](web-bruno-appointments-filter-normalization-gotcha.md) | psychology, web-bruno, appointments, patients, react-query, gotcha | `useAppointments()` depends on `normalizeAppointmentFilters()`, so omitted fields vanish from both the request URL and the query key |
