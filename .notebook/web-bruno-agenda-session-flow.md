...
- `AppointmentForm` keeps the manual-value rule in the component submit path: psychotherapy sessions may save with `value = 0`, but neuromodulation avulsas and package totals still require values greater than zero. The same component also prevents wheel/trackpad changes on its price inputs instead of relying on shared input behavior.
- `SlotDetail` no longer uses WhatsApp as a peer action in the modal. The session-detail UX now keeps the corrective entry points (`Editar`, `Mais ações`, `Excluir`) together in the header, uses the same pill styling for the `Mais ações` trigger and its secondary buttons, and opens those secondary status/recurrence actions in an anchored floating panel instead of expanding the modal body. `packages/web-bruno/src/components/ui/Modal.tsx` now scrolls at the fullscreen wrapper instead of the card itself so anchored panels can escape the card bounds without being clipped.
