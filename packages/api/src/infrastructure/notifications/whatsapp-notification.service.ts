...
import { ChatwootClient, type ChatwootConfig } from './chatwoot.client.js';
...
export function createNotificationService(client: ClientEntity): WhatsAppNotificationService {
  const chatwootConfig: ChatwootConfig | null =
    client.chatwootBaseUrl && client.chatwootToken
      ? {
          baseUrl: client.chatwootBaseUrl,
          apiToken: client.chatwootToken,
...
export class WhatsAppNotificationService {
  ...
  async sendBookingConfirmation(appointment: AppointmentWithDetails): Promise<void> {
    ...
    const cancelUrl = `${this.config.baseUrl}/agendamento/${appointment.cancelToken}`;
    const message = [
